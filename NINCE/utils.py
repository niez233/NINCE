...
def load_oriDat(filePath):
    ...
    # sessDat: (session num: 2218, feature num: 215)
    ...
    for i in range(len(sessFiles)):
        ...
        sessDat = pd.concat([dat, pd.DataFrame(sessDat)], ignore_index=True, sort=False)
    # Replace NaN with 'empty' in owner_cmnt
...
def generate_dict(filePath):
    ...
    # remove html strings in owner_id
    TAG_RE = re.compile(r'<[^>]+>')
    ...
    for i in range(sessDat.shape[0]):
        ...
        uid = [ownerID[i]] + [x.split('   ')[0] for x in comments] # owner and commentator id
        timestamp = [ownerTime[i]] + [x[-20:-1] for x in comments] # post and comment time
        text = [ownerCn[i]] + [x.split('   ')[1][:-34] for x in comments] # post and comment text
