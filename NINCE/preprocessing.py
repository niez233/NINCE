...
# 載入原始資料
filePath = './data/Ins_clean_data/'
# filePath = './data/Vine_clean_data/'
...
all_reviews = list(itertools.chain.from_iterable(reviews))
reviews_freq = len(all_reviews)
print('reviews count in whole data set:', reviews_freq) # reviews count in whole data set: 159277
all_reviews = list(itertools.chain.from_iterable(reviews))
df = pd.DataFrame({'spoken_words': all_reviews})
# 使用正则表达式提取所有中文字符
# chinese_reviews = []
# for review in all_reviews:
#    chinese_text = re.findall(r'[\u4e00-\u9fff]+', review)
#    chinese_reviews.extend(chinese_text)

# 将提取的中文内容存储到数据框中
# df = pd.DataFrame({'spoken_words': chinese_reviews})
...
# brief_cleaning = [re.sub("[^\u4e00-\u9fa5]+", ' ', str(row).lower()) for row in df['spoken_words']]
...
# 設定 word2vec 的模型參數
w2v_model = Word2Vec(min_count=20, window=2, vector_size=300, sample=6e-5, alpha=0.03, min_alpha=0.0007, negative=20, workers=10)
# 建立字典
...
'''
def get_each_review_emb(model, dictDat, MAX_REV_WORD_LEN, MAX_REV_LEN,all_reviews):
    ...
    for i, (pid, messages) in enumerate(dictDat.items()):
        progress = divmod(i+1, int(len(dictDat)*0.1))   # 用于显示进度的
        if progress[1] == 0:
        ...
        # 这里是重新进行了上面词嵌入的那个数据清洗过程
        # 载入所有的评论信息
        df = pd.DataFrame({'review_word': messages.text})
        # 入了字母外的字符都变成了空格，同时字母都变成了小写
        brief_cleaning = [re.sub("[^A-Za-z]+", ' ', str(row).lower()) for row in df['review_word']]
#        brief_cleaning = [re.sub("[^\u4e00-\u9fa5]+", ' ', str(row).lower()) for row in df['review_word']]
        # 用spacy进行清洗
        txt = [cleaning(doc) for doc in nlp.pipe(brief_cleaning, batch_size=5000)]
        df_clean = pd.DataFrame({'clean': txt})
        df_clean = df_clean.dropna().drop_duplicates()
...
#        brief_cleaning = [re.sub("[^\u4e00-\u9fa5]+", ' ', str(row).lower()) for row in df['review']]
        txt = [cleaning(doc) for doc in nlp.pipe(brief_cleaning, batch_size=5000)]
...
# 設定 doc2vec 模型參數
doc_vectorizer = Doc2Vec(min_count=1, window=10, vector_size=300, sample=1e-4, negative=5, workers=8)
# 建立字典
doc_vectorizer.build_vocab(sentence)
# 訓練 doc2vec 模型
doc_vectorizer.train(sentence, total_examples=doc_vectorizer.corpus_count, epochs=1)

'''
利用訓練好的 doc2vec 模型，產生每則貼文的回覆的文本向量，相關參數為 
...
with open('preprocessData/Dat4Model_ins.pickle', 'wb') as f:
...
def PostUserGraph(dictDat):
    ...
    G_dt = pd.DataFrame(columns=['pid','uid'])
    ## 紀錄每則貼文中每則回覆的用戶 uid
    # i.e, pid, uid
    #      1, 'cmacdaddyy'
...
#        brief_cleaning = [re.sub("[^\u4e00-\u9fa5]+", ' ', str(row).lower()) for row in df['review']]
        ...
        df_clean = df_clean.dropna().drop_duplicates()
        g = df_clean[['pid', 'uid']]
        G_dt = pd.concat([G_dt, g], ignore_index=True)
    ...
    # G_dt的append有bug，最终输出的user只有最后一个session的(78个)
...
# 儲存 Multi-Hot Users
with open('preprocessData/multi_hot_users_ins.pickle', 'wb') as f:
